...
Let me start with the Poincaré group. 
The Poincaré group is a mathematical object with certain structure 
and properties. 
It arises as a symmetry group from special relativity. 
Viewed as a set, 
$$
...
$(0, 1_{\mathbb R^4})$.
As an aside, one can obtain a Hamiltonian by
differentiating the time component of the related 
representation $U(te_0, 1_{\mathbb R^4})$ by bringing 
down the first component of the four momentum, which
is the generator of spacetime translations in 
a quantum field theory according to the Wightman axioms (see section 3.1). 

Whereas Section 1.4 on special relativity describes
the physical motivation behind 
deductions that one can make from 
various assumptions (for example, if $L$ is in the proper
Lorentz group, then a forward oriented vector $x \in \mathbb R^4$ 
is mapped back to another forward oriented vector $Lx$)
there is not as much emphasis on 
the topological and differential structure
of the Poincaré group. There is 
a mention of connectivity, but 
it is not described explicitly how to equip 
the Poincare group with a topology. 
In view of the Wightman axioms, for instance, 
what structure is needed on the Poincaré group? 
Since the state transformation law 
is to be implemented using a projective representation
of the proper Poincaré group, this implies at least that
$\mathcal{P}$ can be viewed 
as a topological group: recall that a representation
of a group $G$ on a vector space $V$ is a group homomorphism 
$$\rho: G \to GL(V),$$
